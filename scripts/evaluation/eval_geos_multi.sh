...
benchmark="cvbench"                     # choices: [vsibench, cvbench, blink_spatial,mindcube_full,mindcube_tiny,mmbench_en_dev,embspatial,mmstar,pope,mme,wemath_testmini_reasoning,docvqa_val,embspatial,mmsi_bench,chartqa,ocrbench]
model_path="<PATH_TO_GEOS_CHECKPOINT>"
num_processes=4
stage='geos_multi' #'force_use'、'force_notuse','force_half'(一半vggt) ,geos_multi ("cold_start","stage2-1_rlColdStart"等stage不能在这里使用)
# ---------- 输出与日志 ----------
...
nohup accelerate launch --main_process_port "${PORT}" --num_processes="${num_processes}" -m lmms_eval \
  --model geos_multi \
  --model_args "pretrained=${model_path},use_flash_attention_2=true,max_num_frames=32,max_length=25600,stage=${stage}" \
  --tasks "${benchmark}" \
  --batch_size 1 \
  --log_samples \
