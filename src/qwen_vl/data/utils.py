...
def prepare_image_inputs(image, image_processor, mode="crop"):

    images = load_and_preprocess_images([image], mode=mode)
    geometry_encoder_inputs = images[0].clone()  # 使用clone保持梯度连接
    # print(f"geometry_encoder_inputs in data.utils: {geometry_encoder_inputs, type(geometry_encoder_inputs)}, end geometry encoder inputs")
    merge_size: int = getattr(image_processor, "merge_size")
    patch_size: int = getattr(image_processor, "patch_size")
    # logger.info(f"merge_size and patch_size in data.utils are: {merge_size, patch_size}, end")
