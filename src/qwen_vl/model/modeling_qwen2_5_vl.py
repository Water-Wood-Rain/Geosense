...
class Qwen2_5_VLForConditionalGenerationWithVGGT(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def _process_geometry_features(self, image_embeds, geometry_encoder_inputs):
        ...
        # [DDP Fix] Handle None geometry_encoder_inputs
        if geometry_encoder_inputs is None:
            geometry_encoder_inputs = [None]
        
        batch_size = len(geometry_encoder_inputs)
        geo_embeds = []
        geo_embeds_flat = None
        
        # [DDP Fix] Ensure geometry_encoder is always called to keep DDP buckets in sync
        # This is critical when ddp_find_unused_parameters=False or when using mixed batches
        has_any_geo = any(g is not None and g.shape[0] > 0 for g in geometry_encoder_inputs)
        
        if not has_any_geo and hasattr(self, 'geometry_encoder'):
            # Create dummy input with requires_grad=True to participate in graph
            dummy = torch.zeros(1, 3, 14, 14, device=image_embeds.device, dtype=image_embeds.dtype, requires_grad=True)
            # Forward pass through encoder
            dummy_feat = self.geometry_encoder.encode(dummy).to(image_embeds.dtype)
            # Forward pass through merger
            dummy_feat = dummy_feat.reshape(1, 1, 1, -1)
            dummy_out = self.geometry_merger(dummy_feat)
            
            # [DDP Fix] Connect dummy output to the computation graph
            image_embeds = image_embeds + 0.0 * dummy_out.sum()
            
            del dummy, dummy_feat, dummy_out
        
        for bn in range(batch_size):
            if geometry_encoder_inputs[bn] is not None and geometry_encoder_inputs[bn].shape[0] > 0:
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        pixel_values: Optional[torch.Tensor] = None, #pixel_values.shape torch.Size([720, 1176])pixel_values 指的是已经按视觉打块后的“视觉 token.720 = 该样本里产生的视觉 token 数（和图像被切成多少块有关.   为什么是 1176：Qwen2-VL/2.5-VL 的视觉预处理把每个视觉 token 展平成长度 3 × 2 × 14 × 14 = 1176
        pixel_values_videos: Optional[torch.FloatTensor] = None,
        image_grid_thw: Optional[torch.LongTensor] = None, #tensor([[ 1, 20, 36]], device='cuda:0')image_grid_thw 不是“梯度（gradient）”，而是“网格/栅格（grid）”的意思。image_grid_thw = [[1, 20, 36]]，图像 token 数 = T×H×W = 1×20×36 = 720。
        video_grid_thw: Optional[torch.LongTensor] = None,
        rope_deltas: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.LongTensor] = None,
        second_per_grid_ts: Optional[torch.Tensor] = None,
        geometry_encoder_inputs: Optional[List[torch.Tensor]] = None, #geometry_encoder_inputs[0].shape torch.Size([1, 3, 294, 518])
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            ...
            # [DDP Fix] Initialize vggt_embeds to None to ensure it's always defined
            vggt_embeds = None
            if pixel_values is not None:
                ...
                if os.getenv("Debug", "False")=="whyoom":
                    from remote_pdb import RemotePdb
                    RemotePdb('127.0.0.1', 18457).set_trace()
                # [DDP Fix] Always process geometry features if use_geometry_encoder is True
                # This ensures all ranks follow the same computation graph
                if getattr(self.config, 'use_geometry_encoder', False):
                    # If geometry_encoder_inputs is None, _process_geometry_features will handle it with dummy inputs
                    geometry_encoder_inputs_safe = geometry_encoder_inputs if geometry_encoder_inputs is not None else [None]
                    image_embeds, vggt_embeds = self._process_geometry_features(image_embeds, geometry_encoder_inputs_safe)
                ...
                # [DDP Fix] Check if vggt tokens exist and vggt_embeds is available
                # Only process vggt if both conditions are met to avoid undefined variable errors
                if (input_ids == self.config.vggt_start_token_id).any() and vggt_embeds is not None:
        ...
        if(attention_mask is None or attention_mask.ndim == 2) and self.rope_deltas is None :
            # calculate RoPE index once per generation in the pre-fill stage only
            if (
        ...
        if os.getenv("Debug", "False")=="debug_stage4":
        ...
        if os.getenv("Debug", "False")=="debug_stuck":
            from remote_pdb import set_trace
            set_trace() # you'll see the port number in the logs
        hidden_states = outputs[0]
        ...
        if labels is not None:
            ...
            # 若 labels 还是旧长度（常见情况），需要对齐
            # 注意：由于vggt tokens已经在数据预处理时插入到input_ids中，labels应该已经对齐
            # 如果长度不一致，可能是数据预处理问题，这里暂时跳过对齐（避免使用未定义的N_geo和insert_idx）
            # TODO: 如果需要对齐，需要计算vggt token的数量和位置
            if labels.size(1) != input_ids.size(1):
                # 计算长度差异
                diff_len = input_ids.size(1) - labels.size(1)
                if diff_len > 0:
                    # 如果input_ids更长，在labels末尾padding -100
                    pad_labels = torch.full((B, diff_len), ignore_index, dtype=labels.dtype, device=device)
