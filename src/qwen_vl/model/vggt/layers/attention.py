...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None) -> Tensor:
        ...
        if self.rope is not None:
            # ================= 🔴 修改开始 =================
            # 修复多卡推理时的设备不匹配问题：
            # accelerate 不会自动移动 pos，所以这里必须手动把 pos 移到当前 q 所在的 device
            if pos is not None and pos.device != q.device:
                pos = pos.to(q.device)
            # ================= 🔴 修改结束 =================
            q = self.rope(q, pos)
            k = self.rope(k, pos)
            # ================= 🔴 新增修复代码 =================
            # 解决共享 RoPE 导致的设备漂移问题：
            # 如果 RoPE 被 accelerate 放在了其他卡（如 cuda:0），
            # 这里的输出 q/k 会变成 cuda:0。我们需要把它们拽回当前 v 所在的卡。
