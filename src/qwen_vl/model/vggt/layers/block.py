...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None) -> Tensor:
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            # ================= 🔴 修复代码开始 =================
            # 强制将 Attention 的输出移动到 x 所在的设备，防止多卡推理时的设备不一致报错
            attn_out = attn_residual_func(x, pos=pos)
            if attn_out.device != x.device:
                attn_out = attn_out.to(x.device)
            x = x + attn_out
            
            # 同理，FFN 也加上这个保护（虽然 FFN 通常不会跨卡，但为了保险）
            ffn_out = ffn_residual_func(x)
            if ffn_out.device != x.device:
                ffn_out = ffn_out.to(x.device)
            x = x + ffn_out
            # ================= 🔴 修复代码结束 =================
        return x
