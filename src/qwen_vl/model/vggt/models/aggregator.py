...
class Aggregator(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> Tuple[List[torch.Tensor], int]:
        ...
        # ================= 🔴 修改开始 =================
        # 强制将 mean 和 std 移动到 images 所在的设备
        # 这一步能解决 "Expected all tensors to be on the same device" 报错
        mean = self._resnet_mean.to(images.device)
        std = self._resnet_std.to(images.device)

        # Normalize images and reshape for patch embed
        # 原代码: images = (images - self._resnet_mean) / self._resnet_std
