...
def set_model(model_args, model):
    ...
    if model_args.use_geometry_encoder:
        ...
        # Freeze feature_fusion as it is currently unused
        if hasattr(model, 'feature_fusion'):
            for n, p in model.feature_fusion.named_parameters():
                p.requires_grad = False
...
def train(attn_implementation="flash_attention_2"):
    ...
    logger.info("Global logging setup is done! Starting training...")
    ...
    training_args.ddp_find_unused_parameters = False  # [DDP Fix] Disabled to prevent hangs
    trainer = Trainer(
