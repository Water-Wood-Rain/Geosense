...
def get_train_dataloader(self):
    """
    [DDP Fix] Override get_train_dataloader to ensure drop_last=True
    This ensures all ranks have the same number of steps, preventing synchronization issues.
    [Memory Fix] Added persistent_workers=False and prefetch_factor=2 to prevent memory leak.
    """
    ...
    return DataLoader(
        ...
        drop_last=True,  # [DDP Fix] Drop last incomplete batch to ensure all ranks have same step count
        num_workers=num_workers,
        pin_memory=self.args.dataloader_pin_memory,
        persistent_workers=False,  # [Memory Fix] 禁用持久化 worker，防止内存累积
        prefetch_factor=2 if num_workers > 0 else None,  # [Memory Fix] 减少预取，降低内存占用
    )
